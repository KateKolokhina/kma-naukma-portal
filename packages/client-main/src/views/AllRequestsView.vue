  ...
  <MainLayout>
    ...
    >
      <v-layout align-center pr-4>
        ...
        <v-layout column>
          <div>
            <strong>Помилка при завантаженні сторінки</strong>
          </div>
        </v-layout>
      </v-layout>
    </v-snackbar>
    <v-card-title class="text-h6 text-md-h5 text-lg-h4">Усі замовлення</v-card-title>
        ...
        <tr
          ...
          <td>
            <v-btn class="mx-2"
              ...
              {{ (item.status === "Виконано" || item.status === "Відмінено") ? "Переглянути" : "Переглянути / Редагувати"
              }}
            </v-btn>
          </td>
...
export default {
  name: "AllRequestsViewDepartment",
  components: {
    MainLayout
  },
  data: () => ({
  ...
  async mounted() {
    await axios.post("http://localhost:8080/doc-request/all", {
      page: this.page,
      forDepartment: true,
      sortBy: this.sortBy,
  ...
  methods: {
    ...
    editDocument(item) {
      console.log("open document- " + item.id);
      this.$router.replace({ name: "request-edit", params: { id: item.id } });
    },
